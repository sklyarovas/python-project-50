...
def plain_diff(diff, parents=''):
    ...
    for item in diff:
        ...
        if result == 'modified':
            formated_diff.append(f"Property '{path}' was updated. From {old_val} to {new_val}")
        if result == 'removed':
            formated_diff.append(f"Property '{path}' was removed")
        if result == 'added':
            formated_diff.append(f"Property '{path}' was added with value: {val}")
        if result == 'parent':
